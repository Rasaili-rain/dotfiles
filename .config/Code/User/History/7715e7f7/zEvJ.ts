...
import { GetTransactionsByDheetoIdResponse } from "../types";
...
router.get("/dheeto/:dheetoId/transactions", async (req: Request, res: Response): Promise<GetTransactionsByDheetoIdResponse | undefined> => {
  try {
    const { dheetoId } = req.params;
    ...
    // Count total
    const totalCountQuery = await db.select().from(transactions).where(eq(transactions.dheetoId, dheetoId));
