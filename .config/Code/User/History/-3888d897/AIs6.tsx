...
import { getDheetoById, getTransactionsByDheetoId } from "@/lib/api/api_providers";
...
export default function DheetoDetailPage() {
  ...
  const loadiitem = async (isRefresh = false) => {
    if (!dheeto.id) return;
    try {
      if (!isRefresh) setLoading(true);
      ...
      if (response.success && response.data) {
        ...
        throw new Error(response.success === false ? response.message : "Failed to load iitem");
      }
    } catch (error) {
      Alert.alert("Error", error instanceof Error ? error.message : "Failed to load iitem");
      if (!isRefresh) router.back();
    } finally {
      setLoading(false);
      setRefreshing(false);
        ...
        <View className="mx-4 mt-6">
          <View className="flex-row items-center justify-between mb-3">
            <View className="flex-row items-center gap-2">
              ...
              <Text className="text-sm text-gray-500">({dheeto.totalItemsCount})</Text>
            </View>
