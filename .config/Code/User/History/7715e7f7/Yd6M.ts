...
// ---------------- Utility ----------------
const calculateBalance = (rows: any[]): number =>
  rows.reduce(
    (bal, t) => (t.type === "gave" ? bal - t.amount : bal + t.amount),
    0
  );
...
router.post<
  ...
  AddTransactionResponse,
  AddTransactionBody
>("/dheeto/:dheetoId/add-transaction", async (req, res) => {
  try {
    ...
    if (!["gave", "received"].includes(type))
      return void sendError(
        res,
        "Transaction type must be either 'gave' or 'received'",
        400
      );
    if (amount <= 0)
      return void sendError(res, "Transaction amount must be positive", 400);
    ...
    await db
      .update(dheetos)
      .set({ dheetoBalance: newBalance, updatedAt: new Date() })
      .where(eq(dheetos.id, dheetoId));

    // Update parent person total
    const personId = dheeto.personId;

    const allDheetos = await db
      .select()
      .from(dheetos)
      .where(eq(dheetos.personId, personId));

    const newTotal = allDheetos.reduce(
      (sum, d) => sum + (d.dheetoBalance || 0),
      0
    );

    await db
      .update(persons)
      .set({ totalBalance: newTotal })
...
router.get<
  DheetoIdParams,
  GetTransactionsByDheetoIdResponse
>("/dheeto/:dheetoId/transactions", async (req: Request, res: Response) => {
  try {
    const { dheetoId } = req.params;
...
router.put<
  ...
  UpdateTransactionResponse,
  UpdateTransactionBody
>("/dheeto/:dheetoId/transaction/:transactionId", async (req, res) => {
  try {
    const { dheetoId, transactionId } = req.params;
    const { type, amount, desc } = req.body;

    if (!isValidUUID(dheetoId) || !isValidUUID(transactionId))
      return void sendError(res, "Invalid ID format", 400);

    const txUpdate: Partial<UpdateTransactionBody> = {};

    if (type !== undefined) {
      if (!["gave", "received"].includes(type))
        return void sendError(res, "Invalid type", 400);
      txUpdate.type = type;
    }

    if (amount !== undefined) {
      if (amount <= 0)
        return void sendError(res, "Amount must be positive", 400);
    ...
    if (!existing)
      return void sendError(res, "Transaction not found", 404);
    if (existing.dheetoId !== dheetoId)
      return void sendError(res, "Transaction does not belong to this dheeto", 400);
