...
import { getDheetoById, getItemsByDheetoId, getTransactionsByDheetoId } from "@/lib/api/api_providers";
import { Dheeto, Item, Transaction } from "@/lib/types";

export default function DheetoDetailPage() {
  const { dheeto: DheetoParam } = useLocalSearchParams();

  const currentDheeto: Dheeto | undefined = DheetoParam
    ? (() => {
        const parsed = JSON.parse(DheetoParam as string) as Dheeto;
        return parsed;
      })()
    : undefined;
  if (currentDheeto === undefined) {
    console.error("Dheeto cant be undefined");
    return (
      <View className="flex-1 justify-center items-center bg-gray-50">
        <Text className="text-gray-500 mb-4">Dheeto not found</Text>
  ...
  const [dheeto, setDheeto] = useState<Dheeto>(currentDheeto);
  ...
  const loadDheeto = async (isRefresh = false) => {
    ...
    try {
      ...
      const response = await getDheetoById(dheeto.id);
  ...
  const loadTransactions = async (isRefresh = false) => {
    ...
    try {
      if (!isRefresh) setLoading(true);
      const response = await getTransactionsByDheetoId(dheeto.id);
      if (response.success && response.data) {
        setTransactions(response.data);
      } else {
        throw new Error(response.success === false ? response.message : "Failed to load transaction");
      }
    } catch (error) {
      Alert.alert("Error", error instanceof Error ? error.message : "Failed to load transaction");
      if (!isRefresh) router.back();
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const loadItems = async (isRefresh = false) => {
    ...
    try {
      if (!isRefresh) setLoading(true);
      const response = await getItemsByDheetoId(dheeto.id);
      if (response.success && response.data) {
        setItems(response.data);
      } else {
        throw new Error(response.success === false ? response.message : "Failed to load item");
      }
    } catch (error) {
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    await Promise.all([loadDheeto(true), loadItems(true), loadTransactions(true)]);
  };
  ...
  // Calculate totals
  const totalGoldWeight = items.filter((item) => item.type === "gold").reduce((sum, item) => sum + item.weightInTola, 0);
  const totalSilverWeight = items.filter((item) => item.type === "silver").reduce((sum, item) => sum + item.weightInTola, 0);
  const totalGave = transactions.filter((t) => t.type === "gave").reduce((sum, t) => sum + t.amount, 0);
  ...
  return (
    <View className="flex-1 bg-gray-50">
        ...
        <View className="mx-4 mt-6">
          ...
          {items.length === 0 ? (
            ...
            <View className="gap-2">
              {items.map((item) => (
                <View key={item.id} className={`rounded-lg p-4 border ${item.type === "gold" ? "bg-amber-50 border-amber-200" : "bg-gray-100 border-gray-300"}`}>
                  ...
                  <View className="flex-row items-center justify-between">
                    ...
                    <View className="items-end">
                      <Text className="text-xs text-gray-400">{item.isSettled && item.settledAt ? `settled ${new Date(item.settledAt).toLocaleDateString()}` : "active"}</Text>
        ...
        <View className="mx-4 mt-6 mb-6">
          <View className="flex-row items-center justify-between mb-3">
            <View className="flex-row items-center gap-2">
              ...
              <Text className="text-sm text-gray-500">({transactions.length})</Text>
            </View>
            ...
            <>
              <View className="gap-2 mb-3">
                {transactions.map((transaction) => (
                  <View key={transaction.id} className={`rounded-lg p-4 border ${transaction.type === "gave" ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"}`}>
                    <View className="flex-row items-start justify-between mb-3">
                      <View className="flex-1 pr-3">
      ...
      {/* Modals */}
      {editingItem && <EditItemModal item={editingItem} dheetoId={dheeto.id} onClose={() => setEditingItem(null)} onUpdate={loadDheeto} />}
      {editingTransaction && <EditTransactionModal transaction={editingTransaction} dheetoId={dheeto.id} onClose={() => setEditingTransaction(null)} onUpdate={loadDheeto} />}
      {showAddItem && <AddItemModal dheetoId={dheeto.id} onClose={() => setShowAddItem(false)} onUpdate={loadDheeto} />}
      {showAddTransaction && <AddTransactionModal dheetoId={dheeto.id} onClose={() => setShowAddTransaction(false)} onUpdate={loadDheeto} />}
