...
export default function PersonDetailRoute() {
  ...
  const goToIndividualDheetoPage = (dheeto: Dheeto) =>
    router.push({
      pathname: "/person/dheeto/[id]",
      params: { id: dheeto.id, dheeto: JSON.stringify(dheeto) },
    });
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh} />}>
          ...
          {person.totalDheetosCount > 0 && dheetos !== null && (
            <View className="mx-4 space-y-3">
              {dheetos.map((dheeto) => (
                <DheetoCard key={dheeto.id} dheeto={dheeto} person={person} onPress={() => goToIndividualDheetoPage(dheeto)} />
              ))}
            </View>
          )}
